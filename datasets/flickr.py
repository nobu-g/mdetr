...
def build_mmdialogue(image_set, args):
    ...
    if args.test:
        ann_file = Path(args.mmdialogue_ann_path) / f"final_flickr_{identifier}_test.json"
        img_dir = Path(args.mmdialogue_img_path) / "test"
    else:
        ann_file = Path(args.mmdialogue_ann_path) / f"final_flickr_{identifier}_{image_set}.json"
        img_dir = Path(args.mmdialogue_img_path) / f"{image_set}"
